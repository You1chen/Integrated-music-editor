"""Configuration manager — JSON-based persistence for app settings."""
...
class ConfigManager:
    ...
    def get_lyric(self) -> str:
        """Read the draft file from AppData, or an empty string when absent."""
        try:
    ...
    def set_lyric(self, text: str) -> None:
        """Write the draft file to AppData."""
    ...
    def get_reaction_time_ms(self) -> int:
        """Reaction time offset in milliseconds (default 100)."""
        return self.get_preferences().get("reactionTimeMs", 100)

    def get_undo_seek_back_seconds(self) -> float:
        """Seconds to seek back after undoing a timestamp (default 3.0)."""
    ...
    def overwrite_lrc(self, text: str) -> tuple[bool, str]:
        """Overwrite the source LRC file with *text*; returns (success, message)."""
    ...
    def get_draft_path(self) -> str:
        """Return the draft file path (AppData/draft.lrc)."""
    ...
    def get_last_volume(self) -> Dict[str, Any]:
        """Restore the last-session output volume and mute state."""
        return {
    ...
    def set_last_volume(self, volume: float, muted: bool) -> None:
        """Persist the output volume and mute state."""
        cfg = self._load_config()
    ...
    def get_last_play_mode(self) -> int:
        """Restore the last-session playback mode."""
        return int(self._load_config().get("lastPlayMode", 0))
    ...
    def get_api_configs(self) -> "list[dict[str, str]]":
        """Return the decrypted API configs that have url, api_key and model."""
    ...
    def toggle_playlist_like(self, path: str) -> bool:
        """Toggle the liked state of a song; returns the new state."""
